import { connectOnce, OutboxModel, OfferModel, markOffersNotified, withMongoRetry } from "./db.js";
...
export const handler = async (): Promise<void> => {
  ...
  const events = await withMongoRetry(
    "notify.loadOutboxBatch",
    () => OutboxModel.find({}, null, { sort: { createdAt: -1 }, limit: BATCH_SIZE }).lean(),
  );
  ...
  const offerDocs = await withMongoRetry(
    "notify.loadOfferNotificationState",
    () => OfferModel.find({ guid: { $in: guids } }, { guid: 1, notifiedAt: 1 }).lean(),
  );
  ...
  if (pending.length === 0) {
    await withMongoRetry(
      "notify.deleteAlreadyNotifiedOutboxEvents",
  ...
  await Promise.all([
    withMongoRetry("notify.deleteOutboxBatch", () => OutboxModel.deleteMany({ _id: { $in: ids } })),
